    ...
    if resize:
        ...
        if image.size != (480, 480):
            ...
            if width < crop_size or height < crop_size:
                raise ValueError(
                    f"Image size {image.size} is smaller than the crop size {crop_size}."
                )
...
def set_custom_clip_embeddings(
    ...
    def process(sample: dict):
        ...
        if args.model_name == "radir":
            input_data = {"text": sample["reports"], "image_paths": sample["image_files"]}
            _embeds = model.encode(**input_data)
            if retrieval_modality == "image":
                embeds = _embeds[0]
            elif retrieval_modality == "text":
                embeds = _embeds[1]
            else:
                embeds = torch.cat(_embeds, dim=1)
